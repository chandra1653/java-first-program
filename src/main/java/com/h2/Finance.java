...
public class Finance {
    ...
    public final static Map<String, String> commandToUsage = Map.of(BEST_LOAN_RATES, "usage: bestLoanRates",
    ...
    public static void main (String[] args){
        String command = args[0];
            if(!commandToUsage.containsKey(command)){
                System.out.println(command + ": command not found");
                return;
            }
            boolean isValidCommand = validateCommandArguments(args);
            if(!isValidCommand){
                System.out.println(commandToUsage.get(args[0]));
                return;
            }
